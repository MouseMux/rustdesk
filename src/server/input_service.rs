...
pub fn sync_mousemux_ids(conn_id: i32) {
    log::info!(
        "MouseMux v2.2 protocol: sync_mousemux_ids() called for conn_id {}",
        conn_id
    );
    ...
    if let Ok(mut enigo) = ENIGO.lock() {
        ...
        log::info!(
            "MouseMux v2.2 protocol: Retrieved IDs from windows_mousemux for conn_id {}: mouse={:?}, keyboard={:?}",
            conn_id,
            mouse_id,
            keyboard_id
        );
        ...
        log::info!(
            "MouseMux v2.2 protocol: Updated MAIN PROCESS Enigo instance for conn_id {}",
            conn_id
        );
        ...
        log::error!(
            "MouseMux v2.2 protocol: Failed to lock ENIGO mutex for conn_id {}",
            conn_id
        );
    }
}
...
pub fn handle_mouse_simulation_(evt: &MouseEvent, conn: i32) {
    ...
    // Upstream 1.4.9 split handle_mouse_ into a wrapper plus this simulation
    // function; the guard belongs here, where input is actually injected, not in
    // the wrapper (which also does whiteboard rendering and touches no ENIGO state).
    #[cfg(windows)]
    let _input_guard = INPUT_SERIALIZE.lock().unwrap();
    ...
    #[cfg(windows)]
    {
        ...
        crate::mm_debug!("MouseMux v2.2 protocol: handle_mouse_() - Set current_conn_id to {}", conn);
    }
    #[cfg(target_os = "macos")]
...
fn map_keyboard_mode(evt: &KeyEvent) {
    crate::mm_debug!("MouseMux v2.2 protocol: map_keyboard_mode() called - chr={}, down={}", evt.chr(), evt.down);
    #[cfg(windows)]
    crate::platform::windows::try_change_desktop();
...
fn legacy_keyboard_mode(evt: &KeyEvent) {
    crate::mm_debug!("MouseMux v2.2 protocol: legacy_keyboard_mode() called - union={:?}, down={}", evt.union, evt.down);
    #[cfg(windows)]
    crate::platform::windows::try_change_desktop();
...
pub fn handle_key_with_conn(evt: &KeyEvent, conn: i32) {
    crate::mm_debug!("MouseMux v2.2 protocol: handle_key_with_conn() called for conn_id {}, down={}, mode={:?}, union={:?}",
    ...
    #[cfg(windows)]
    {
        ...
        crate::mm_debug!("MouseMux v2.2 protocol: handle_key_with_conn() - Set current_conn_id to {} and rdev keyboard_id to {} (0x{:X})",
    ...
    crate::mm_debug!("MouseMux v2.2 protocol: handle_key_with_conn() - About to call handle_key_() for conn {}", conn);
    handle_key_(evt);
    crate::mm_debug!("MouseMux v2.2 protocol: handle_key_with_conn() - Returned from handle_key_() for conn {}", conn);
}

pub fn handle_key_(evt: &KeyEvent) {
    crate::mm_debug!("MouseMux v2.2 protocol: handle_key_() called - mode={:?}, down={}", evt.mode, evt.down);
    if EXITING.load(Ordering::SeqCst) {
        return;
    }
